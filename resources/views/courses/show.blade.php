...
@section('title', $course->title . ' - نسور')

@php
    $shareUrl = url()->current();
    $shareTitle = $course->title;
    $shareDescription = $course->desc ? Str::limit($course->desc, 160) : 'كورس من منصة نسور للتعلم الإلكتروني';
    $shareImage = $course->images->count() > 0 
        ? url('storage/' . $course->images->first()->image_path) 
                ...
                <div class="col-lg-10 text-center">
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb justify-content-center">
                            ...
                            <li class="breadcrumb-item"><a class="text-white" href="{{ route('courses.index') }}">الكورسات</a></li>
                            <li class="breadcrumb-item text-white active" aria-current="page">{{ Str::limit($course->title, 30) }}</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="row g-5">
                ...
                <div class="col-lg-8">
                    ...
                    @if($course->images->count() > 0)
                        <div class="mb-5 wow fadeInUp" data-wow-delay="0.1s">
                            <a href="{{ asset('storage/' . $course->images->first()->image_path) }}" 
                               class="glightbox" 
                               data-gallery="course-gallery">
                                <img class="img-fluid rounded" 
                                     src="{{ asset('storage/' . $course->images->first()->image_path) }}" 
                                     alt="{{ $course->title }}" 
                    ...
                    <div class="mb-4 wow fadeInUp" data-wow-delay="0.2s">
                        ...
                        <div class="d-flex flex-wrap gap-3 mb-3">
                            ...
                            @if($course->time)
                                <span class="text-muted">
                                    <i class="fa fa-clock text-primary me-2"></i>{{ $course->time }}
                                </span>
                            @endif
                            @if($course->presenter)
                                <span class="text-muted">
                        ...
                        <div class="mb-5 wow fadeInUp" data-wow-delay="0.5s">
                            ...
                            <div class="ratio ratio-16x9">
                                @php
                                    ...
                                    $youtubeId = '';
                                    if (preg_match('/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]{11})/', $course->youtube_url, $matches)) {
                                        $youtubeId = $matches[1];
                                    }
                                @endphp
                                @if($youtubeId)
                                ...
                                @else
                                    <a href="{{ $course->youtube_url }}" target="_blank" class="btn btn-primary">شاهد الفيديو على يوتيوب</a>
                                @endif
                            </div>
                        </div>
                    @endif
                    ...
                    @if($course->images->count() > 1)
                        <div class="mb-5 wow fadeInUp" data-wow-delay="0.6s">
                            ...
                            <div class="row g-3">
                                @foreach($course->images->skip(1) as $index => $image)
                                    <div class="col-md-4">
                ...
                <div class="col-lg-4">
                    <div class="bg-light p-4 rounded mb-4 wow fadeInUp" data-wow-delay="0.3s">
                        <h5 class="mb-3">معلومات الكورس</h5>
                        <ul class="list-unstyled">
                            @if($course->presenter)
                                <li class="mb-3">
                                    <i class="fa fa-user-tie text-primary me-2"></i>
                                    <strong>المقدم:</strong> {{ $course->presenter }}
                                </li>
                            @endif
                            @if($course->date)
                                <li class="mb-3">
                                    <i class="fa fa-calendar text-primary me-2"></i>
                                    <strong>التاريخ:</strong> {{ $course->date->format('Y-m-d') }}
                                </li>
                            @endif
                            @if($course->time)
                                <li class="mb-3">
                                    <i class="fa fa-clock text-primary me-2"></i>
                                    <strong>الوقت:</strong> {{ $course->time }}
                                </li>
                            @endif
                            <li class="mb-3">
                                <i class="fa fa-calendar-check text-primary me-2"></i>
                                <strong>تاريخ النشر:</strong> {{ $course->created_at->format('Y-m-d') }}
                            </li>
                        </ul>
                    </div>
